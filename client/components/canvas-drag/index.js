...
dragGraph.prototype = {
    ...
    paint() {
        ...
        this.ctx.translate(-this.centerX, -this.centerY);
        console.log(this.centerX, this.centerY, this.rotate)

        // 渲染元素
        if (this.type === 'text') {
            this.ctx.fillText(this.text, this.centerX, this.centerY);
        } else if (this.type === 'image') {
            console.log('ele', this.x, this.y, this.w, this.h)
...
Component({
    ...
    methods: {
        ...
        start(e) {
            isMove = false; // 重置移动标识（借鉴）
            const {x, y} = e.touches[0]; // 获取焦点坐标
            this.tempGraphArr = []; // 临时图形组
            let lastDelIndex = null; // 记录最后一个需要删除的索引

            // 通过点击动作，给画布中的所有元素添加操作事件
            this.drawArr && this.drawArr.forEach((item, index) => {
                const action = item.isInGraph(x, y); // 判断是否再当前图形范围内（碰撞检测）
                if (action) {
                    item.action = action;
                    this.tempGraphArr.push(item); // 更新每一个元素对应的操作属性
                    // 保存点击时的坐标
                    this.currentTouch = {x, y};// （借鉴）
                    if (action === 'del') {
